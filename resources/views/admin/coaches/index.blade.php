    ...
    <div class="space-y-6">
        ...
        <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/30 shadow-[0px_4px_20px_rgba(23,32,51,0.02)] overflow-hidden">
            <div class="p-5 border-b border-outline-variant/30 bg-surface/50 flex items-center justify-between">
                <form method="GET" class="flex items-center gap-2 w-full">
                    <div class="relative flex-1 min-w-0">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama pelatih..." class="w-full pl-10 pr-4 py-2 bg-surface-container-low border border-outline-variant/50 rounded-full focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary font-body-sm text-body-sm transition-all">
                    </div>
            ...
            <div class="md:hidden divide-y divide-outline-variant/30">
                    ...
                    <div class="p-4 hover:bg-surface-container-low/50 transition-colors">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3 min-w-0">
                                @if ($coach->photo)
                                    <img src="{{ Storage::url($coach->photo) }}" class="w-10 h-10 rounded-full object-cover shrink-0">
                                @else
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        <p class="font-label-md text-label-md text-on-surface truncate">{{ $coach->name }}</p>
                                        <span class="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-label-sm text-label-sm {{ $coach->is_active ? 'bg-[#E8F5E9] text-[#2E7D32]' : 'bg-error-container text-on-error-container' }}">
                                            {{ $coach->is_active ? 'Aktif' : 'Nonaktif' }}
                                        </span>
                                    </div>
                                    <p class="font-body-sm text-body-sm text-outline truncate">{{ $coach->email }}</p>
                                </div>
                            </div>
                            <a href="{{ route('admin.coaches.edit', $coach) }}" class="shrink-0 p-2 -m-1 rounded-lg text-outline hover:text-primary hover:bg-surface-container-low transition-colors" title="Edit">
                        ...
                        <div class="mt-3 pt-3 border-t border-outline-variant/30 flex items-center justify-between gap-3">
                            <div>
                                <p class="font-label-sm text-label-sm text-outline">Izin Penilaian</p>
                                <p class="font-body-sm text-body-sm {{ $coach->can_assess_developments ? 'text-[#2E7D32]' : 'text-on-surface-variant' }} mt-0.5">
                                    {{ $coach->can_assess_developments ? 'Boleh mengisi penilaian' : 'Tidak boleh mengisi penilaian' }}
                                </p>
                            </div>
                            <form action="{{ route('admin.coaches.toggle-development', $coach) }}" method="POST"
                                ...
                                @csrf @method('PATCH')
                                <button type="submit" class="shrink-0 inline-flex items-center gap-2 group" title="{{ $coach->can_assess_developments ? 'Matikan izin penilaian' : 'Aktifkan izin penilaian' }}">
                ...
                @empty
                    <div class="p-8 text-center font-body-sm text-body-sm text-outline">Belum ada data pelatih.</div>
                @endforelse
            </div>
