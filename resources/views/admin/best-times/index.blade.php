<x-sidebar-layout>
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="font-headline text-headline-lg-mobile md:text-headline-lg text-on-surface">Best Time Siswa</h2>
                <p class="font-body-sm text-body-sm text-outline mt-1">Rekor pribadi waktu renang siswa, dikelola lewat kelas Kompetitif.</p>
            </div>
        </div>
        ...
        <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/30 shadow-[0px_4px_20px_rgba(23,32,51,0.02)] overflow-hidden">
            <div class="p-5 border-b border-outline-variant/30 bg-surface/50 flex flex-col sm:flex-row sm:items-center gap-4 sm:justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="font-headline text-headline-sm text-on-surface">Data Best Time</h3>
                </div>
            ...
            <div class="md:hidden divide-y divide-outline-variant/30">
                @forelse ($students as $student)
                    @php
                        $latest = $student->bestTimes->first();
                    @endphp
                    <div class="p-4 hover:bg-surface-container-low/50 transition-colors">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <span class="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-label-sm text-label-sm bg-primary-container text-on-primary">
                                <span class="material-symbols-outlined text-[14px]">timer</span>
                                {{ $student->bestTimes->count() }}x
                            </span>
                        </div>
                        <div class="mt-3">
                            <a href="{{ route('admin.classes.best-times.history', [$latest->schoolClass, $student]) }}" class="inline-flex items-center gap-1 text-primary font-label-md text-label-md hover:underline">
                ...
                @empty
                    <div class="p-10 text-center font-body-sm text-body-sm text-outline">Belum ada data best time.</div>
                @endforelse
            </div>
                    ...
                    <tbody class="divide-y divide-outline-variant/30">
                        @forelse ($students as $student)
                            ...
                            <tr class="hover:bg-surface-container-low/50 transition-colors">
                                ...
                                <td class="px-4 py-3 font-body-sm text-body-sm text-on-surface">{{ $student->bestTimes->count() }}x</td>
                                <td class="px-4 py-3">
                                    <a href="{{ route('admin.classes.best-times.history', [$latest->schoolClass, $student]) }}" class="inline-flex items-center gap-1 text-primary font-label-md text-label-md hover:underline">
                                        <span class="material-symbols-outlined text-[16px]">timer</span>
                                        Lihat Riwayat
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center font-body-sm text-body-sm text-outline">Belum ada data best time.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
